...
- El nombre del paquete en `Cargo.toml` es siempre `name = "app"` → el
  ...
  visible del producto (Retro Darts 420). No asumir que el nombre del exe
  coincide con el nombre de la app.
...
- Si una app siembra datos por defecto en `localStorage` con un check tipo
  ...
  defecto crecen en versiones futuras, **los usuarios existentes nunca van
  a recibir los datos nuevos**, porque el store ya no está vacío. Cualquier
  lógica de seed debe ser incremental/versionada, nunca un check binario de
  "vacío o no".
- Un instalador o limpiador de caché **nunca** debe borrar `Local Storage`,
  ...
  del usuario (partidas, puntajes, etc.). Solo limpiar `Cache`,
...
- Retro Darts 420 no usa Vite: es HTML/CSS/JS plano en `www/`, empaquetado
  directo con `frontendDist: "../www"` en `tauri.conf.json`, sin
  `beforeBuildCommand`. Si en algún momento se agrega un bundler, recordar
  que scripts clásicos (`<script src="...">` sin `type="module"`) no se
